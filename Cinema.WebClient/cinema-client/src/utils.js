...
export function getDayMonth(dateString) {
    ...
    const day = date.getDate(); // виправлено getDay() на getDate()
    const month = date.toLocaleString('en', { month: 'long' });
    return `${day} ${month}`;
}

export function getAverageRating(ratingArray) {
    if (ratingArray.length === 0) return 0; // виправлено умову
...
function divideSessionsByDate(sessions) {
    return sessions.reduce((accumulator, session) => {
        const date = dayjs(session.date).format('YYYY-MM-DD'); // форматування дати
...
function formatDateHeaders(sessions) {
    sessions = Object.entries(sessions)
        .sort((a, b) => new Date(a[0]) - new Date(b[0])) // змінено сортування
...
export function validateLoginData(userData) {
    ...
    let status = { invalid: false };
    if (!passwordValid) status.password = true, status.invalid = true;
    if (!usernameValid) status.username = true, status.invalid = true;
